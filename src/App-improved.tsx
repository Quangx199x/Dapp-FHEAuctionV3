...
function App() {
  ...
  const connectWallet = async () => {
    try {
      ...
      addLog('success', 'Contract initialized');
    ...
    <div className="container">
      ...
      <div className="grid">
        ...
        <div className="panel glow">
          <div className="panel-header">&gt; &gt; INIT WALLET SERVICE & FHE SDK</div>
          ...
          <div className="info-item">
            <div className="info-label">[NETWORK] Target Network:</div>
            <div>Sepolia (11155111)</div>
          </div>

          <div className="info-item">
            <div className="info-label">[FHE SDK]</div>
            <div style={{ color: fheStatus === 'Ready' ? 'var(--green)' : 'var(--green-dark)' }}>
              {fheStatus}
            </div>
          </div>

          <button className="btn" onClick={connectWallet} disabled={isConnected}>
            {isConnected ? 'WALLET CONNECTED' : 'CONNECT WALLET'}
          </button>

          {isConnected && (
            <button className="btn" onClick={initFHE} disabled={fheStatus === 'Ready' || fheStatus === 'Initializing...'}>
              {fheStatus === 'Ready' ? 'FHE SDK READY' : fheStatus === 'Initializing...' ? 'INITIALIZING...' : 'INITIALIZE FHE SDK'}
            </button>
          )}
        </div>

        {/* Auction Data - IMPROVED VERSION */}
        <div className="panel glow">
          <div className="panel-header">&gt; &gt; AUCTION DATA FEED (FHE PROTECTED)</div>
          ...
          <div className="info-grid">
            ...
            <div>
              ...
              <div className="badge">{auctionData.state}</div>
            </div>
            ...
            <div>
              ...
              <div className="info-value">
                0.000 ETH <span className="badge-encrypted">🔒 ENCRYPTED</span>
              </div>
            </div>
          ...
          <div className="info-item" style={{ marginTop: '1rem' }}>
            <div className="info-label">[CURRENT BLOCK]</div>
            <div className="info-value">{auctionData.currentBlock.toLocaleString()}</div>
          </div>

          <div className="info-item">
            <div className="info-label">[END BLOCK]</div>
            <div className="info-value">{auctionData.endBlock.toLocaleString()}</div>
          </div>
          ...
          {auctionData.estimatedEndTime && (
            <div className="info-item">
              <div className="info-label">[ESTIMATED END]</div>
      ...
      <div className="footer">
        ...
        <div>Powered by Zama FHE | Network: Sepolia | Contract: {CONTRACT_ADDRESS.slice(0, 6)}...{CONTRACT_ADDRESS.slice(-4)}</div>
      </div>
    </div>
  );
}
